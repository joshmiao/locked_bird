...
class Paddle {
  constructor (_main) {
    let p = {
      ...
      h: 22,                                               // 图片高度
      speed: 8,                                           // x轴移动速度
      ballSpeedMax: 5+Number(sessionStorage.getItem("locked_bird_break_bricks-result")),// 小球反弹速度最大值
...
class Ball {
  constructor (_main) {
    let b = {
      ...
      w: 18,                                // 图片宽度
      h: 18,                                // 图片高度
      speedX: 1+Number(sessionStorage.getItem("locked_bird_break_bricks-result"))/3, // x轴速度
      speedY:5+Number(sessionStorage.getItem("locked_bird_break_bricks-result")), // y轴速度
...
class Scene {
  ...
  initBlockList () {
    ...
    for (let item of this.blockList)
    {
      ...
      {
        if (list.type === 1) {
          let obj = new Block(list.x, list.y,1);
          arr.push(obj)
          x=1
        } 
        else 
